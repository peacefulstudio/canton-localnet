...
- `<name>` carries semantic meaning. There are two scopes for `<name>`:
  ...
  - **Party hint** — supplied per-consumer via `canton-localnet.yaml`. Names the *validator itself*, defaulting to its own slot name so the shipped artifact carries no consumer domain. Consumers remap it as they like, e.g. `a-validator-1` → party hint `featuredapp-validator-1`; terraform-tests uses `a-validator-1` → `tf-validator-1`. This is distinct from the *users* hosted on a validator, which are named separately via that slot's `parties` list — a scenario naming its users Alice and Bob (letter-aligned to `a-validator-1`/`b-validator-1`) leaves those validators' party hints untouched.
